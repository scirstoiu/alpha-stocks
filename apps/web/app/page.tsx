...
function LatestNews() {
  ...
  if (items.length === 0) {
    return (
      <Card>
        <h3 className="text-sm font-medium text-gray-500 mb-2">Latest News</h3>
  ...
  return (
    <Card className="overflow-hidden p-0">
      <h3 className="px-4 py-2 text-sm font-medium text-gray-500 border-b border-gray-200 bg-gray-50">Latest News</h3>
      <div className="divide-y divide-gray-100">
        {items.map((n) => (
          <a key={n.id} href={n.url} target="_blank" rel="noopener noreferrer" className="block px-4 py-2.5 hover:bg-gray-50">
            ...
            <p className="text-xs text-gray-400 mt-0.5">{n.source} &middot; {timeAgo(n.publishedAt)}</p>
          </a>
        ))}
      </div>
