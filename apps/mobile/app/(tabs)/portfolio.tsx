  ...
  return (
    <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.statsHeaderRow}>
        <Text style={[styles.statsHeaderText, { flex: 1.2, textAlign: 'left' }]}>Symbol</Text>
        <Text style={[styles.statsHeaderText, { flex: 1, textAlign: 'right' }]}>Value</Text>
        <Text style={[styles.statsHeaderText, { flex: 0.7, textAlign: 'right' }]}>Weight</Text>
        ...
        return (
          <View key={pos.symbol}>
            <TouchableOpacity onPress={() => toggleExpand(pos.symbol)} style={styles.statsRow}>
              ...
              <Text style={[styles.statsWeight, { flex: 0.7, textAlign: 'right' }]}>{totalValue > 0 ? ((pos.value / totalValue) * 100).toFixed(1) : 0}%</Text>
            ...
            {isExp && breakdown.map((b) => (
              <View key={`${pos.symbol}-${b.portfolioId}`} style={styles.statsSubRow}>
                <Text style={[styles.statsSubName, { flex: 1.2 }]}>{b.portfolioName}</Text>
                <Text style={[styles.statsSubValue, { flex: 1, textAlign: 'right' }]}>{formatCurrency(b.value)}</Text>
                <Text style={[styles.statsSubWeight, { flex: 0.7, textAlign: 'right' }]}>{pos.value > 0 ? ((b.value / pos.value) * 100).toFixed(1) : 0}%</Text>
